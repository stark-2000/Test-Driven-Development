...
PID_Controller my_pid(0.5, 0.01, 0.011, 0.1);

TEST(Compute_PID, Default_Values) {
  EXPECT_NEAR(pid_default.compute_pid(0.0, 10.0), 10, 0.2);
  EXPECT_NEAR(pid_default.compute_pid(0.0, -10.0), -10, 6);
  EXPECT_NEAR(pid_default.compute_pid(5.0, -10.0), -10, 6);
}

TEST(Compute_PID, Client_Input) {
  EXPECT_NEAR(my_pid.compute_pid(0.0, 10.0), 10, 0.2);
  EXPECT_NEAR(my_pid.compute_pid(0.0, -10.0), -10, 6);
  EXPECT_NEAR(my_pid.compute_pid(5.0, -10.0), -10, 6);
}
